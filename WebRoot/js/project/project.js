$(document).ready(function() {
	$.ajax({
		...
		success: function(data) {
			...
			if(len > 0) {
				for(var i = 0; i < len; i++) {
					var addDrawingHTML = "<tr onclick='project_detail("+data[i][0]+")'>" +
						"<td>" + data[i][0] + "</td>" +
						"<td>" + data[i][1] + "</td>" +
						"<td>" + data[i][2] + "</td>" +
						"<td>" + data[i][3] + "</td>" +
...
function new_Project(){
	window.location.href= "jsp/project/index.jsp";
}
...
function project_detail(obj){
	window.location.href= "jsp/project/project_detail.jsp?id="+obj;
}